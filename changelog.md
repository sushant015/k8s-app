...
### Added
- **Wrapper Chart for Loki:** Created `k8s-helm-charts/monitoring/loki-monitoring` wrapper chart referencing `loki-stack` (v2.10.3) as a dependency to control log-aggregation resources.
...
- **Directory Layout Restructuring:** Reorganized `k8s-helm-charts/` by grouping charts into dedicated `monitoring/` and `apps/` folders to improve repository cleanliness.

### Changed
- **Grafana Integration:** Registered Loki as an automatic Grafana DataSource in `k8s-helm-charts/monitoring/grafana-monitoring/values.yaml` pointing to `http://loki:3100`.
